...
public class EnvServer {
    ...
    private void createNewWorld(MissionInit missionInit, Long seed) {
        ...
        // ========== TimeInitialCondition Support ==========
        // Apply StartTime from ServerInitialConditions after world creation
        applyTimeConditions(missionInit);
        // ========== End TimeInitialCondition Support ==========
    }

    /**
     * Apply time conditions from ServerInitialConditions
     * Added for TimeInitialCondition support
     */
    private void applyTimeConditions(MissionInit missionInit) {
        try {
            ServerInitialConditions serverInit = getServerInit(missionInit);
            if (serverInit == null || serverInit.getTime() == null) {
                return;
            }
            
            Time timeConfig = serverInit.getTime();
            Integer startTime = timeConfig.getStartTime();
            Boolean allowPassage = timeConfig.isAllowPassageOfTime();
            
            if (startTime != null) {
                LOGGER.info("*** TimeInitialCondition: Setting world time to " + startTime + " ***");
                ...
                if (server != null) {
                    for (ServerWorld world : server.getWorlds()) {
                        world.setDayTime(startTime);
                        LOGGER.info("*** Set day time to " + startTime + " for world: " + world.getDimensionKey().getLocation() + " ***");
                    }
                } else {
                    LOGGER.warn("*** MinecraftServer not ready, cannot set time ***");
                }
            }
            
            if (allowPassage != null && !allowPassage) {
                LOGGER.info("*** TimeInitialCondition: Disabling daylight cycle ***");
                Minecraft mc = Minecraft.getInstance();
                MinecraftServer server = mc.getIntegratedServer();
                if (server != null) {
                    server.getGameRules().get(GameRules.DO_DAYLIGHT_CYCLE).set(false, server);
                } else {
                    LOGGER.warn("*** MinecraftServer not ready, cannot disable daylight cycle ***");
                }
            }
        } catch (Exception e) {
            LOGGER.error("Error applying time conditions: ", e);
        }
    }
