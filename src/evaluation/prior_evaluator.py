...
class Steve1PriorEvaluator:
    ...
    def __init__(
        ...
        logger.info(f"✓ Prior VAE: {prior_weights} 已加载")
        
        # 加载训练样本的视觉嵌入（可以稍后通过 .success_visuals 属性设置）
        # success_visuals 结构: {task_id: {'success_visual_embeds': [np.ndarray, ...]}}
        self.success_visuals = {}
        if success_visuals_path and Path(success_visuals_path).exists():
            logger.info(f"加载视觉嵌入: {success_visuals_path}")
            with open(success_visuals_path, 'rb') as f:
                self.success_visuals = pickle.load(f)
            logger.info(f"✓ 已加载 {len(self.success_visuals)} 个任务的视觉嵌入")
        elif success_visuals_path:
            # 只有明确指定路径但不存在时才警告
            logger.warning(f"⚠️ 视觉嵌入文件不存在: {success_visuals_path}")
        # 如果 success_visuals_path 为 None，表示将通过属性手动设置（由 eval_framework 从 train_samples 加载）
        
        # 加载指令变体（可以稍后通过 .instruction_variants 属性设置）
        ...
        elif instruction_variants_path:
            # 只有明确指定路径但不存在时才警告
            logger.warning(f"⚠️ 指令变体文件不存在: {instruction_variants_path}")
        # 如果 instruction_variants_path 为 None，表示将通过属性手动设置，不打印警告
