"""
MineRL 自定义环境和 Wrappers
============================

提供 MineRL 环境的自定义配置和包装器：
- MineRLHarvestWrapper: 支持自定义奖励计算（Harvest/Combat任务）
- MineRLHarvestDefaultEnvSpec: 可配置的 MineRL 环境规格
...
class MineRLHarvestWrapper(gym.Wrapper):
    ...
    def __init__(self, env, reward_config: List[Dict], reward_rule: str = "any"):
        ...
        if self.task_type == "inventory":
            ...
            logger.info(f"MineRLHarvestWrapper: Inventory模式")
            ...
            self.prev_stats = {}
            logger.info(f"MineRLHarvestWrapper: Event模式")
    ...
    def _get_entity_kills(self, kill_stats: dict, entity_type: str) -> int:
        """
        从kill_stats中获取指定实体的击杀数
        
        尝试多种实体名称格式以兼容MineRL的不同返回格式:
        - 原始名称: zombie
        - 首字母大写: Zombie
        - 带命名空间: minecraft:zombie
        - 全大写: ZOMBIE
        
        Args:
            kill_stats: MineRL返回的kill_entity统计字典
            entity_type: 配置中的实体类型 (如 "zombie")
        ...
        name_variants = [
            entity_type,                              # zombie
            entity_type.capitalize(),                 # Zombie
            entity_type.title(),                      # Zombie (同上，但对多词更好)
            f"minecraft:{entity_type}",               # minecraft:zombie
            entity_type.upper(),                      # ZOMBIE
            entity_type.lower(),                      # zombie (确保小写)
        ]
        
        for name in name_variants:
            if name in kill_stats:
                return kill_stats[name]
        
        # 如果都没找到，检查是否有部分匹配
        entity_lower = entity_type.lower()
        for key, value in kill_stats.items():
            if entity_lower in key.lower():
                logger.debug(f"实体名称部分匹配: {entity_type} → {key}")
                return value
    ...
    def _calculate_event_reward(self, obs) -> float:
        ...
        for entity_type, cfg in self.event_targets.items():
            ...
            if event_type == "kill_entity":
                ...
                # 首次打印观察到的kill_entity键（调试用）
                if not hasattr(self, '_kill_stats_logged'):
                    #logger.info(f"[DEBUG] kill_entity 观察空间: {kill_stats if kill_stats else '(空)'}")
                    #logger.info(f"[DEBUG] 正在监控实体: {entity_type}")
                    self._kill_stats_logged = True
                
                # 尝试多种实体名称格式（MineRL可能使用不同格式）
                current_kills = self._get_entity_kills(kill_stats, entity_type)
...
def register_minerl_harvest_default_env():
    ...
    try:
        gym.register(
            ...
            entry_point='src.envs.minerl_wrappers:_minerl_harvest_default_env_entrypoint'
        )
        logger.info("✓ MineRLHarvestDefaultEnv-v0 已注册（DefaultWorldGenerator）")
    except gym.error.Error:
        pass
    
    # 注册别名 MineRLHarvestEnv-v0（向后兼容）
    try:
        gym.register(
            id='MineRLHarvestEnv-v0',
            entry_point='src.envs.minerl_wrappers:_minerl_harvest_default_env_entrypoint'
        )
        logger.debug("✓ MineRLHarvestEnv-v0 已注册（别名）")
    except gym.error.Error:
        pass
