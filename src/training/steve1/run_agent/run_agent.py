...
from steve1.config import PRIOR_INFO, DEVICE, DATA_DIR
...
def run_agent(prompt_embed, gameplay_length, save_video_filepath,
    ...
    for _ in tqdm(range(gameplay_length)):
        ...
        # Render the game window if requested
        if render:
            env.render()
...
def generate_text_prompt_videos(prompt_embeds, in_model, in_weights, cond_scale, gameplay_length, save_dirpath, render=False):
    for name, prompt_embed in prompt_embeds.items():
        ...
        if not os.path.exists(save_video_filepath):
            run_agent(prompt_embed, gameplay_length, save_video_filepath,
                      in_model, in_weights, None, cond_scale, render)
        else:
            print(f'Video already exists at {save_video_filepath}, skipping...')


def generate_visual_prompt_videos(prompt_embeds, in_model, in_weights, cond_scale, gameplay_length, save_dirpath, render=False):
    for name, prompt_embed in prompt_embeds.items():
...
if __name__ == '__main__':
    ...
    parser.add_argument('--in_model', type=str, default=os.path.join(DATA_DIR, 'weights/vpt/2x.model'))
    parser.add_argument('--in_weights', type=str, default=os.path.join(DATA_DIR, 'weights/steve1/steve1.weights'))
    parser.add_argument('--prior_weights', type=str, default=os.path.join(DATA_DIR, 'weights/steve1/steve1_prior.pt'))
    ...
    parser.add_argument('--save_dirpath', type=str, default=os.path.join(DATA_DIR, 'generated_videos/'))
    parser.add_argument('--custom_text_prompt', type=str, default=None)
    parser.add_argument('--render', action='store_true', help='显示游戏窗口（会降低速度）')
    ...
    if args.custom_text_prompt is not None:
        ...
        generate_text_prompt_videos(custom_prompt_embeds, args.in_model, args.in_weights, args.text_cond_scale,
                                    args.gameplay_length, args.save_dirpath, args.render)
    else:
