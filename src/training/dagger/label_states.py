...
class StateLabeler:
    ...
    def __init__(self, failure_window=10, random_sample_rate=0.1):
        ...
        self.action_mapping = {
            ...
            # 视角控制 - 方向键（特殊处理）
            82: [0, 0, 0, 8, 12, 0, 0, 0],    # ↑ 向上看
            84: [0, 0, 0, 16, 12, 0, 0, 0],   # ↓ 向下看
            81: [0, 0, 0, 12, 8, 0, 0, 0],    # ← 向左看
            83: [0, 0, 0, 12, 16, 0, 0, 0],   # → 向右看
            ...
            # 组合动作
            'q': [1, 0, 1, 12, 12, 0, 0, 0],  # 前进+跳跃 ⭐
            'e': [1, 0, 0, 12, 12, 3, 0, 0],  # 前进+攻击 ⭐
            'r': [1, 0, 1, 12, 12, 3, 0, 0],  # 前进+跳跃+攻击
            
            # 视角复位
            'p': [0, 0, 0, 12, 12, 0, 0, 0],  # 视角置为中心 (12, 12) ⭐
            
            # 特殊
            'x': 'pass',  # 保持策略动作（认为策略正确）⭐
            'c': None,    # 跳过此状态 ⭐
            'z': 'undo',  # 撤销上一个标注 ⭐
        }
    ...
    def display_state(self, state_info, current_idx, total):
        ...
        # 放大显示（更大的窗口）
        display_img = cv2.resize(display_img, (960, 720))
        ...
        # 顶部信息框
        cv2.rectangle(display_img, (5, 5), (955, 100), (0, 0, 0), -1)
        cv2.rectangle(display_img, (5, 5), (955, 100), (255, 255, 255), 2)
        ...
        info_lines = [
            ...
            f"Policy Action: {action_description}"
        ]
        
        for i, line in enumerate(info_lines):
            cv2.putText(
                display_img, line, (10, 25 + i*25),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 1
            )
        
        # 底部快捷键说明框
        help_y_start = 620
        cv2.rectangle(display_img, (5, help_y_start), (955, 715), (0, 0, 0), -1)
        cv2.rectangle(display_img, (5, help_y_start), (955, 715), (255, 255, 255), 2)
        ...
        for i, line in enumerate(help_text):
            cv2.putText(
                display_img, line, (10, help_y_start + 25 + i*25),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (100, 200, 255), 1
            )
        
        cv2.imshow('DAgger State Labeling - Expert Annotation', display_img)
    ...
    def label_states(self, states_to_label):
        ...
        print("")
        print("视角控制:")
        print("  P            - 视角置为中心 (12, 12) ⭐")
        print("")
        print("特殊操作:")
        print("  X            - 保持策略动作（认为策略正确）⭐")
        print("  C            - 跳过此状态")
        print("  Z            - 撤销上一个标注 ⭐")
        print("  ESC          - 完成标注并保存")
        print(f"{'='*60}\n")
        ...
        while current_idx < len(states_to_label):
            ...
            if key in self.action_mapping:
                ...
                if key_char == 'z':
                    # 撤销 - 修复：使用索引删除而不是 remove()
                    if self.undo_stack and len(self.labeled_data) > 0:
                        self.undo_stack.pop()
                        self.labeled_data.pop()  # 删除最后一个
                ...
                if key_char not in self.action_mapping:
            ...
            if action == 'pass':
                ...
                print(f"  ✓ [{current_idx+1}/{len(states_to_label)}] KEEP (保持策略: {action_desc})")
                current_idx += 1
                ...
                # 显示我们标注的新动作
                action_name = self._get_action_name(key if key in [81, 82, 83, 84] else key_char)
