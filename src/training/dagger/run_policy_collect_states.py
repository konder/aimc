...
def collect_policy_states(
    ...
    for ep in range(num_episodes):
        # 每个episode创建新环境（避免内存泄漏）
        print(f"Episode {ep+1}/{num_episodes} [创建环境] ", end="", flush=True)
    ...
    print(f"完成: {success_count + failure_count} | 跳过: {skipped_count}")
    print(f"成功: {success_count} ({success_count/(num_episodes-skipped_count)*100:.1f}%)" if (num_episodes-skipped_count) > 0 else "成功: 0")
    print(f"失败: {failure_count} ({failure_count/(num_episodes-skipped_count)*100:.1f}%)" if (num_episodes-skipped_count) > 0 else "失败: 0")
